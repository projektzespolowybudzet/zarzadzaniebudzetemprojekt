...
public class UserDaoImpl extends BaseDaoImpl<User> implements UserDao {
  ...
  private static final String loginSql =
    "update pf_user set last_login = :last_login where id = :id";
  ...
  private static final String loginExistsSql =
    "select id from pf_user where LOWER(login) = ? and id != ?";
  ...
  public boolean loginExists(String login, long id) {
    List<Long> list = getJdbcTemplate()
      .queryForList(
        loginExistsSql,
        new Object[] { login.toLowerCase(), id },
        Long.class
      );
    return list.size() > 0;
  }

  private static final String emailExistsSql =
    "select id from pf_user where email = ? and id != ?";

  @Override
  public boolean emailExists(String email, long id) {
    List<Long> list = getJdbcTemplate()
      .queryForList(emailExistsSql, new Object[] { email, id }, Long.class);
    return list.size() > 0;
  }
  ...
  private static final String getUsersSql =
    "select * from pf_user order by login";
    ...
    getJdbcTemplate()
      .update(
        ...
        new Object[] {
          ...
          user.isEnabled(),
          user.getRole().name(),
          DateUtils.convert(user.getCreatedAt()),
          user.getChangedBy(),
          DateUtils.convert(user.getChangedAt()),
        }
      );
  }
  ...
  private static final String getUserSql =
    "select * from pf_user u where u.id = :id";
  ...
  private static final String changePasswordSql =
    "update pf_user set changed_by = :changed_by, " +
  ...
  public void changePassword(String encryptedPassword, long userId) {
    ...
    params.put(DBUtils.changed_at, new Timestamp(System.currentTimeMillis()));
    params.put(UserMapper.LAST_PASSWORD_CHANGE, new Date());
    parameterJdbcTemplate.update(changePasswordSql, params);
  }

  private static final String getUserByNameSql =
    "select * from pf_user u where u.login = :login";
  ...
  private static final String updateIconSql =
    "update pf_user set icon = :icon where id = :id";
  ...
  private static final String updateProfileSql =
    "update pf_user " +
    "set login = :login, first_name = :first_name, last_name = :last_name, email = :email, " +
  ...
  private static final String getUserByEmailSql =
    "select * from pf_user u where u.email = :email";
  ...
  private static final String createVerificationTokenSql =
  ...
  private static final String updateVerificationTokenSql =
    "update verification_token " +
    "set token = :token, expiry_date = :expiry_date, expired = :expired where id = :id";

  private static final String deleteVerificationTokenSql =
  ...
  private static final String getTokenSql =
    "select * from verification_token where user_id = :user_id and token_type = :token_type";
