...
import am.jsl.dolarek.dao.transaction.mapper.*;
...
public class TransactionDaoImpl
  extends BaseDaoImpl<Transaction>
  ...
  /**
   * This method is called by the database to search transactions.
   */
  @Autowired
  ...
  public TransactionSearchResult search(TransactionSearchQuery searchQuery) {
    ...
    /**
     * Calculate the search totals if the count is zero.
     */
  ...
  private static final String totalColumnsSql =
    "t.transaction_type, sum(t.amount) as total, acc.symbol ";
  private static final String totalEndSql =
    " group by t.transaction_type, acc.currency, acc.symbol) res " +
    "where res.total > 0 order by res.transaction_type, res.symbol";

  private List<TransactionListTotalDTO> calculateSearchTotals(
  ...
  private String createWhereClouse(
    ...
    if (searchQuery.getCategoryId() != 0) {
      where.append(
        " and (t.category_id = :category_id or cat.parent_id = :category_id) "
      );
  ...
  private static final String deleteSql =
    "delete from transaction where user_id = :user_id and id = :id";
  ...
  public void create(Transaction transaction) {
    ...
    params.put(
      DBUtils.transaction_date,
      DateUtils.convert(transaction.getTransactionDate())
    );
  ...
  private static final String createBatchSql =
    "insert into transaction " +
    "(id,  account_id, category_id, amount, status, " +
    "transaction_type, transaction_source, transaction_date, description, user_id) " +
    "values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
  ...
  private static final String getDetailsSql =
  ...
  public TransactionDetailsDTO getDetails(long id, long userId) {
    ...
    TransactionDetailsDTO detailsDTO = parameterJdbcTemplate.queryForObject(
      getDetailsSql,
      params,
      detailsMapper
    );
  ...
  private static final String searchByCategorySql =
    "select * from (select cat.name as category, " +
   ...
   *
   * @param query - the search query to be used for the search.
   */
  @Override
  public List<TransactionByCategoryDTO> search(
    TransactionByCategorySearchQuery query
  ) {
    Map<String, Object> params = new HashMap<>();
    ...
    finalSql.append(
      " group by t.category_id) as t where t.total > 0 order by t.total desc"
    );

    return parameterJdbcTemplate.query(
      finalSql.toString(),
      params,
      trByCategoryDTOMapper
    );
  }

  private static final String getAmountSql =
    "select amount from transaction where id = :id and user_id = :user_id";
  ...
  public void update(Transaction transaction) {
    ...
    /**
     * Delete the transfer if the transaction is a transfer type.
     */
    if (!transaction.isTransferType()) {
      ...
      /**
       * Delete the transfer if the transaction is the previous transaction.
       */
    ...
    /**
     * If the transaction is a transfer type then create a new transfer.
     */
    if (transaction.isTransferType()) {
  ...
  private static final String deleteTransferSql =
    "delete from transfer where transaction_id = :transaction_id";
  ...
  private static final String updateTransferSql =
    "update transfer set target_account_id = :target_account_id, rate = :rate, " +
  ...
  private static final String createTransferSql =
    "insert into transfer (id,  transaction_id, target_account_id, rate, converted_amount) " +
  ...
  private static final String getSql =
    "select * from transaction where user_id = :user_id and id = :id";
  ...
  private static final String getTransactionTransferSql =
    "select * from transfer where transaction_id = :transaction_id";

  private Transfer getTransfer(long transactionId) {
    Map<String, Object> params = new HashMap<>();
    params.put(DBUtils.transaction_id, transactionId);
    return parameterJdbcTemplate.queryForObject(
      getTransactionTransferSql,
      params,
      transferRowMapper
    );
  }

  private static final String getTransactionTransferIdSql =
    "select id from transfer where transaction_id = :transaction_id";

  private Long getTransferId(long transactionId) {
    Map<String, Object> params = new HashMap<>();
    params.put(DBUtils.transaction_id, transactionId);
    return parameterJdbcTemplate.queryForObject(
      getTransactionTransferIdSql,
      params,
      Long.class
    );
  }

  private static final String getTransactionTypeSql =
    "select transaction_type from transaction where id = :id";

  private short getTransactionType(long id) {
    Map<String, Object> params = new HashMap<>();
    params.put(DBUtils.id, id);
    return parameterJdbcTemplate.queryForObject(
      getTransactionTypeSql,
      params,
      Short.class
    );
  }
