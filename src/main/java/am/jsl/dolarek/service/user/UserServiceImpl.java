package am.jsl.dolarek.service.user;

import am.jsl.dolarek.dao.user.UserDao;
...
import am.jsl.dolarek.service.EmailService;
...
public class UserServiceImpl
  ...
  /**
   * Obiekt dostępu do danych użytkownika.
   */
  ...
  @Transactional(propagation = Propagation.REQUIRED)
  @Caching(evict = { @CacheEvict(value = "userByName", key = "#user.login") })
  ...
  public void sendPasswordResetMail(
    ...
    if (user != null) {
      VerificationToken verificationToken = userDao.getToken(
        user.getId(),
      ...
      verificationToken.setTokenType(
        VerificationTokenType.PASSWORD_RESET.getValue()
      );
  ...
  @Override
  public VerificationToken checkToken(
    long userId,
    String token,
  ...
  public void resetPassword(PasswordResetDTO passwordResetDTO)
    ...
    VerificationToken verificationToken = checkToken(
      userId,
      token,
      VerificationTokenType.PASSWORD_RESET
    );
  ...
  @Transactional(propagation = Propagation.REQUIRED)
  @Override
  public void register(User user, Locale locale, String contextPath)
    throws Exception {
    ...
    VerificationToken verificationToken = userDao.getToken(
      user.getId(),
      VerificationTokenType.NEW_ACCOUNT
    );
    ...
    verificationToken.updateToken(token);
    verificationToken.setTokenType(
      VerificationTokenType.NEW_ACCOUNT.getValue()
    );
    ...
    final String registrationConfirmLink =
      contextPath +
      "/user-public/confirm-registration?id=" +
      user.getId() +
      "&token=" +
      token;
  ...
  @Transactional(propagation = Propagation.REQUIRED)
  @Override
  public void confirmRegistration(Long userId, String token)
