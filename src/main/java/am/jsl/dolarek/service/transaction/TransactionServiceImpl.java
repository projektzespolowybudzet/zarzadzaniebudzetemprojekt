...
import am.jsl.dolarek.dao.account.AccountDao;
import am.jsl.dolarek.dao.transaction.TransactionDao;
import am.jsl.dolarek.domain.account.Account;
import am.jsl.dolarek.domain.transaction.Transaction;
import am.jsl.dolarek.domain.transaction.TransactionStatus;
...
import am.jsl.dolarek.util.NumberUtils;
...
public class TransactionServiceImpl
  extends BaseServiceImpl<Transaction>
  ...
  /**
   * Obiekt dostępu do danych transakcji
   */
  private TransactionDao transactionDao;

  /**
   * Obiekt dostępu do danych konta
   */
  ...
  public TransactionSearchResult search(TransactionSearchQuery searchQuery) {
    ...
    SimpleValueWrapper cachedSearchQuery = (SimpleValueWrapper) cache.get(
      CACHE_KEY_SEARCH_QUERY + userId
    );

    if (
      cachedSearchQuery == null || !cachedSearchQuery.get().equals(searchQuery)
    ) {
    ...
    if (searchQuery.isCalculateTotals()) {
      ...
      result.setTotals(
        (List<TransactionListTotalDTO>) cache
          .get(CACHE_KEY_SEARCH_TOTALS + userId)
          .get()
      );
    }
    return result;
  ...
  @Caching(
    evict = {
      @CacheEvict(
        value = CACHE_TRANSACTION_SEARCH,
        key = "'s' + #transaction.userId"
      ),
      @CacheEvict(
        value = CACHE_TRANSACTION_SEARCH,
        key = "'t' + #transaction.userId"
      ),
      @CacheEvict(
        value = CACHE_TRANSACTION_SEARCH,
        key = "'ct' + #transaction.userId"
      ),
    }
  )
  @Override
  ...
  private void updateAccountBalance(Transaction transaction) {
    ...
    TransactionType type = TransactionType.get(
  ...
  @Caching(
    evict = {
      @CacheEvict(
        value = CACHE_TRANSACTION_SEARCH,
        key = "'s' + #addTransactionsDTO.userId"
      ),
      @CacheEvict(
        value = CACHE_TRANSACTION_SEARCH,
        key = "'t' + #addTransactionsDTO.userId"
      ),
      @CacheEvict(
        value = CACHE_TRANSACTION_SEARCH,
        key = "'ct' + #addTransactionsDTO.userId"
      ),
    }
  )
  @Override
  ...
  @Override
  public TransactionByCategoryResultDTO search(
    ...
    if (cachedSearchQuery != null && cachedSearchQuery.get() != null) {
      searchCategoryMap =
        (Map<Integer, TransactionByCategoryResultDTO>) cache
          .get(cacheKey)
          .get();

      if (searchCategoryMap != null) {
        TransactionByCategoryResultDTO searchResult = searchCategoryMap.get(
          queryHashCode
        );
    ...
    double total = list
      .stream()
      .mapToDouble(TransactionByCategoryDTO::getTotal)
      .sum();
    ...
    result.setList(list);
    result.setTotalStr(
      NumberUtils.formatAmount(total) + (symbol != null ? symbol : "")
    );
  ...
  @Autowired
  public void setDaos(
    @Qualifier("transactionDao") TransactionDao transactionDao,
