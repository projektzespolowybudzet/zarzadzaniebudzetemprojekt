...
public class DatabaseDumpServiceImpl implements DatabaseDumpService {
  ...
  private static final AppLogger log = new AppLogger(
    DatabaseDumpServiceImpl.class
  );

  @Value("${dolarek.db.export.enabled}")
  private boolean exportEnabled = false;

  @Value("${dolarek.db.export.cmd}")
  private String exportCmd;

  @Value("${dolarek.db.export.file.prefix}")
  private String exportFilePrefix;

  @Value("${dolarek.db.export.filedir}")
  private String exportFiledir;

  @Value("${dolarek.db.export.files_count}")
    ...
    try {
      ...
      if (files != null) {
        ...
        if ((filesCount - exportFilesCount) > 0) {
          ...
          Arrays
            .stream(files, exportFilesCount, filesCount)
            .forEach(File::delete);
        }
      }
    } catch (Exception e) {
