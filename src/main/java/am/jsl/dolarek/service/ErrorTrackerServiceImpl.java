...
public class ErrorTrackerServiceImpl implements ErrorTrackerService {
  ...
  protected static final AppLogger log = new AppLogger(
    ErrorTrackerServiceImpl.class
  );

  private static final String DEFAULT_SUBJECT = "Error Tracker";

  @Value("${dolarek.error_tracker.send.email}")
  private boolean sendEmail;

  @Value("${dolarek.error_tracker.email}")
  private String trackerEmail;

  @Value("${dolarek.error_tracker.duplicate.errors.send.interval}")
  ...
  public void sendError(Exception e) {
    ...
    try {
      ...
      exceptionCache
        .entrySet()
        .removeIf(entry ->
          entry.getValue().plusHours(duplicatesSendInterval).isBefore(now)
        );
