...
public class EmailServiceImpl implements EmailService {
  ...
  public void sendPasswordResetMail(
    ...
    final String htmlContent =
      this.emailTemplateEngine.process(RESET_PASSWORD_TEMPLATE, ctx);
  ...
  public void sendRegistrationMail(
    String recipientEmail,
    String registrationConfirmLink,
    Locale locale
  ) throws MessagingException {
    if (!mailEnabled) {
      log.debug("Mailing is disabled");
    ...
    // Utwórz treść HTML za pomocą thymeleaf
    final String htmlContent =
      this.emailTemplateEngine.process(REGISTRATION_TEMPLATE, ctx);
    message.setText(htmlContent, true);
  ...
  @Override
  public void sendEmail(String email, String subject, String emailText)
    throws MessagingException {
    if (!mailEnabled) {
      log.debug("Mailing is disabled");
      return;
